...
### 每 7 天一次：結構審視（Day 9 新增）

**不看功能，只看「東西有沒有放對地方、有沒有越來越難維護」。**
功能正確性由測試負責，這個檢查負責的是結構。

檢查項目：

- [ ] **檔案位置**：有沒有類別放在不屬於它的套件？
- [ ] **命名**：類別名說的事，跟它實際做的事一致嗎？（尤其是測試）
- [ ] **重複**：同一段邏輯出現第三次了嗎？（第二次先忍，第三次才抽）
- [ ] **根目錄**：有沒有暫存檔、IDE 產生的垃圾？**有沒有含機密的檔案？**
- [ ] **文件漂移**：`API_DESIGN.md`、`DATABASE_DESIGN.md` 跟實作還一致嗎？
- [ ] **設定一致性**：測試環境與正式環境的設定值相同嗎？

產出寫成 `docs/REVIEW_DAYnn.md`，並用 🔴 / 🟡 / 🟢 標示嚴重度。

> **為什麼要獨立成一個儀式**：這些問題單獨看都不嚴重，
> 所以永遠排不進「今天要做的事」。累積三個月之後就變成「這個專案好難改」。
