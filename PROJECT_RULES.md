...
> **你看得懂的醜程式碼，勝過你看不懂的漂亮程式碼。**

這條規則的地位等同優先序——因為看不懂的程式碼，
在面試時等於不存在（你講不出來），在維護時等於負債（你不敢改）。

### 具體要求

| 要求 | 說明 |
|---|---|
| **一行只做一件事** | 拒絕「一個運算式串接五個操作」的寫法，即使它比較短 |
| **寧可多寫三行** | 明確的 `if` 勝過隱晦的串接。優化寫法是懂了原理之後的事 |
| **命名要能讀出意圖** | `detectReuseOrFail()` 勝過 `check()` |
| **註解寫「為什麼」不寫「做什麼」** | 「做什麼」應該由程式碼本身表達 |

### 驗收標準

**能不能口頭解釋每一行。** 解釋不出來的程式碼一律重寫成看得懂的版本，
不論它多有效率、多優雅。

### 已知的例外（待清理）

目前程式碼中的 Javadoc 密度**遠高於真實專案的合理範圍**。
那些是**教材**，不是產品程式碼。

**Day 20 文件整理時刪減至合理密度**，只保留「為什麼這樣做」與
「這裡有陷阱」兩類註解。
