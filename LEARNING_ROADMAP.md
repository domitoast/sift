...
### A8.1 交易出錯之後就回不來了（Day 17，踩過）
- **掌握度**：🟢（親手踩，四個測試同時爆）
- **症狀**：
  ```
  UnexpectedRollback: Transaction silently rolled back because it has been marked as rollback-only
  ```
- **原因**：交易中一旦發生資料庫錯誤，那個交易就被標記為 rollback-only。
  **在方法裡 catch 住不會讓它復活。** 方法正常回傳後 Spring 想 commit，
  但那個交易已經不能 commit 了
- **關鍵對照**：

  | | catch 之後做什麼 | 結果 |
  |---|---|---|
  | `UserService.register()` | **往外丟例外** | ✅ 回滾，符合預期 |
  | 原本的 `saveIfNew` | **正常回傳 false** | ❌ UnexpectedRollback |

- **規則**：**在 `@Transactional` 方法內部 catch 住資料庫例外、然後正常回傳，是行不通的。**
  catch 必須在交易外面
- **本專案的修法**：`saveIfNew` 拿掉 `@Transactional`。
  Spring Data 的 repository 方法本身就是交易性的，
  `saveAndFlush` 會開自己的交易，失敗時自行回滾，例外才乾淨地傳出來
- **⚠️ 同型 bug 的發現**：`FetchJobService.startJob` 從 Day 15 就有一樣的形狀，
  但只有「兩個實例同時跑排程」才會觸發，所以一直沒被發現。
  **一個 bug 被測試抓到時，要去找同樣形狀的其他地方**
...
- **和 dedup 的差別**：
  ...
  | | |
  |---|---|
  | **dedup** | 「怎麼做」——挑掉已經存在的（**手段**） |
  | **idempotency** | 「做完具備什麼性質」——重跑不會有副作用（**性質**） |

  因為做了 dedup，抓取流程才是 idempotent 的。
  但兩者不綁定：`DELETE ... WHERE expires_at < now()` 是 idempotent，
  卻完全沒有去重這個動作
- **HTTP 的對照**：`GET` `PUT` `DELETE` 是 idempotent，**`POST` 不是**——
  這就是「重複送出表單會產生重複訂單」的原因
- **本專案的實測**：排程跑過約 50 輪 × 30 篇 = 1500 篇，
  實際不同的文章只有數十篇。**沒有 idempotency 就是 1400 筆垃圾 + 1400 次 LLM 費用**
- **面試考點**：怎麼設計 idempotency key？（本專案用 `(source_id, content_hash)`）

### C2. Retry 與 exponential backoff（Day 19）
- **掌握度**：🟢（實作 + 8 題測試）
- **三種做法的差別**：

  | 做法 | 後果 |
  |---|---|
  | 立刻重試、無上限 | 對方剛說「你太快了」，你的回應是打得更快。自己的執行緒也卡死 |
  | 固定間隔 5 秒 | 100 篇同時失敗 → 同時重試 → **尖峰只是往後移**（thundering herd） |
  | **exponential backoff** | 60 → 120 → 240 秒。**對方越修不好，我們打得越少** |

- **實作的核心只有一行**：`baseDelay * (1L << retryCount)`
- **jitter（抖動）**：在等待時間上加 ±20% 隨機，讓同時失敗的項目散開。
  沒有它，backoff 只是把整個尖峰往後移一分鐘
- **上限要有,而且不能太大**：本專案設 3 次
  - 每次重試都花錢
  - 1+2+4 = 7 分鐘，夠讓大部分限流恢復
  - 7 分鐘還不行，大概就不是暫時性問題
- **⭐ 狀態怎麼走**：暫時性失敗回到 `NEW` 而不是 `FAILED`——
  **`FAILED` 的語意是「結束了」，但暫時性失敗還沒結束，它只是還沒成功**
- **存「下次何時重試」而非「已等待多久」**：後者要有人一直數，
  服務重啟就忘了；前者是絕對時間，重啟照樣有效（V1 欄位註解，Day 3 就寫了）
- **⚠️ 撞到的 bug**：`jitterRatio = 0` 時 `nextDouble(0.0, 0.0)` 會丟
  `bound must be greater than origin`。**0 是合法設定值，邊界要特別處理**
- **怎麼測有隨機性的程式碼**：先把隨機關掉（`jitterRatio = 0`）驗證確定的邏輯，
  再另外寫一題專門驗證「隨機真的有作用」
- **面試考點**：「外部 API 失敗你怎麼處理？」——
  答得出 backoff 是及格，答得出 jitter 和上限的理由才是加分
...
### C7. Dedup 策略（Day 17）
- **掌握度**：🟢（實作完成，ADR-004）
- **判斷「同一篇」的依據是內容雜湊，不是網址**：

  ```
  今天  example.com/post?id=5
  明天  example.com/post?id=5&utm_source=rss     ← 網址變了，內容沒變
  ```
  用網址比對會每天多存一份、每天多付一次摘要費用
- **insert-or-ignore，不是「先查再寫」**：直接 INSERT，撞到唯一約束就跳過
- **⭐ 什麼時候用哪一種**：
  ...
  | 重複時 | **要給錯誤訊息** | 跳過就好 |
  | 一次幾筆 | 1 | 30（其中 25 重複） |
  | 寫法 | 先查再寫 + 第二道防線 | **純 insert-or-ignore** |

  **「先查」存在的唯一理由是產生好的錯誤訊息。沒有人在看的時候，那個成本不值得付。**
- **雜湊的實作細節**：`title + "\n" + content`，那個 `\n` 不是排版——
  少了它，`("ab","c")` 和 `("a","bc")` 會算出同一個值
- **已知代價**：原文改一個字就視為新文章（ADR-004 明確接受）
...
### D4.2 雜湊 vs 加密：怎麼選（Day 18）
- **掌握度**：⭐（三種都實作過，說得出為什麼各自不同）
- **判斷只有一句話：你需不需要把原始的東西拿回來？**

  | 資料 | 需要還原嗎 | 用什麼 | 為什麼 |
  |---|---|---|---|
  | 密碼 | ❌ 只要比對 | **BCrypt** | 而且要**慢**——密碼是人想的，快的雜湊等於送分 |
  | refresh token | ❌ 只要比對 | **SHA-256** | 256 bits 隨機值，暴力破解本來就不可行，不需要放慢 |
  | **LLM API key** | ✅ **要拿去呼叫別人的 API** | **AES-GCM 加密** | 雜湊了就永遠拿不回來 |

- **⭐ 為什麼「能用雜湊就不要用加密」**：

  | | 資料庫外洩 | 資料庫**和金鑰**都外洩 |
  |---|---|---|
  | 雜湊 | 要一個一個暴力破解 | 一樣——**沒有金鑰這回事** |
  | 加密 | 看不懂的字串 | **全部瞬間變明文** |

  而金鑰通常就放在同一台伺服器上。**加密多出一整類風險。**
- **反向的錯誤更嚴重**：把密碼拿去加密，是很多大公司出過的事——
  「加密聽起來比雜湊高級」是錯的直覺
- **BCrypt 自動加 salt**：同一個密碼、兩個人，雜湊值不同。
  防彩虹表，也防「一眼看出這五個人用同一個密碼」
- **面試考點**：「密碼為什麼不能加密？」

### D4.3 AES-GCM 與金鑰管理（Day 18）
- **掌握度**：🟢（實作 + 7 題測試）
- **IV（初始化向量）**：每次加密都要一個新的隨機值。
  少了它，同樣的明文永遠產生同樣的密文——
  **不用解密就能看出「這兩個使用者用同一把 key」**。與 BCrypt 的 salt 同理
- **⚠️ GCM 特別怕 IV 重複**：不只是洩漏一點資訊。同一把金鑰 + 同一個 IV
  加密兩份東西，攻擊者可以推算出產生認證標記的子金鑰——
  **防竄改的功能整個失效，可以偽造出「通過驗證」的假密文**
- **GCM 的認證標記**：密文被改過時直接拒絕，而不是回傳一堆亂碼。
  這叫 authenticated encryption
- **儲存格式**：`Base64( [IV 12 bytes][密文][認證標記 16 bytes] )`。
  IV 不用保密，但解密要用，所以跟密文存在一起
- **金鑰管理**：
  - 不能放程式碼（會進 git）
  - 不能放資料庫（**等於把鑰匙掛在鎖上**）
  - 放環境變數，且**不給預設值**——找不到就啟動失敗
  - **一把鑰匙只做一件事**：`jwt.secret` 外洩 → 可偽造 token；
    `encryption.secret` 外洩 → 所有 API key 變明文。
    共用一把的話，輪替其中一個用途會連帶弄壞另一個
- **fail fast**：金鑰長度不對要在**建構子**就爆，不要等第一次加密

### D4.4 敏感資料的三條防線（Day 18，ADR-003）
- **掌握度**：🟢（三條各有一題測試）
- **不得明文儲存** → 加密後入庫。**驗證方式是直接查資料庫，不是看 API 回應**——
  加密壞掉時 API 一樣會回遮罩，但資料庫裡躺著明文
- **不得回傳前端** → 只給遮罩 `sk-a...mnop`。
  露出頭尾各四碼是為了讓使用者確認「貼對了」（Stripe、AWS 的通用做法）
- **不得出現在日誌** → **覆寫 `record` 的 `toString()`**
  ...
  只要有人加一行 `log.debug("request={}", request)`，那把 key 就躺在 log 檔裡
- **⚠️ 覆寫 `toString()` 只擋日誌，擋不了 JSON**——
  Jackson 讀的是欄位存取方法，不看 `toString()`。
  擋 JSON 的是「回應永遠用 `UserResponse`，而它沒有那個欄位」。**兩條路要分別堵**
- **為什麼 log 特別危險**：log 會被複製、被送到第三方服務、**被截圖貼進聊天室**。
  **能看到 log 的人通常比能看到資料庫的人多很多**
